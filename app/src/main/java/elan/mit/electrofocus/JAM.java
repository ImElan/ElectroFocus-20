...
public class JAM extends AppCompatActivity {
    private DatabaseReference mJAM;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Just A Minute");
        ...
        mJAM = FirebaseDatabase.getInstance().getReference().child("Events").child("JAM").child("General");
        mJAM.keepSynced(true);
        mJAM.addValueEventListener(new ValueEventListener() {
            @Override
        ...
        ViewButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v)
            {
                if(isNetworkAvailable())
                {
                    if(check.equals("yes"))
                    {
                        Intent SelectionListIntent = new Intent(JAM.this,EventSelectionList.class);
                        SelectionListIntent.putExtra("event","JAM");
                        SelectionListIntent.putExtra("toolbar","Just A Minute");
                        startActivity(SelectionListIntent);
                    }
                    else if(check.equals("anyone"))
                    {
                        Toast.makeText(JAM.this,"Anyone Can Attend "+title+": )",Toast.LENGTH_LONG).show();
                    }
                    else
                    {
                        if(title==null)
                        {
                            Toast.makeText(JAM.this, "Please wait while fetching the data", Toast.LENGTH_LONG).show();
                        }
                        else
                        {
                            Toast.makeText(JAM.this, "Selection List For " + title + " is not yet updated", Toast.LENGTH_LONG).show();
                        }
                    }
                }
                else
                {
                    Toast.makeText(JAM.this,"Please Check Your Internet Connection",Toast.LENGTH_LONG).show();
                }
            }
        });
    }
