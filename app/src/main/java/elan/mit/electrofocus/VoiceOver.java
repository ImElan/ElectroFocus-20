...
public class VoiceOver extends AppCompatActivity {
    private DatabaseReference mVoiceOver;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Voice Over");
        ...
        mVoiceOver = FirebaseDatabase.getInstance().getReference().child("Events").child("VoiceOver").child("General");
        mVoiceOver.keepSynced(true);
        mVoiceOver.addValueEventListener(new ValueEventListener() {
            @Override
        ...
        ViewButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v)
            {
                if(isNetworkAvailable())
                {
                    if(check.equals("yes"))
                    {
                        Intent SelectionListIntent = new Intent(VoiceOver.this, EventSelectionList.class);
                        SelectionListIntent.putExtra("event","VoiceOver");
                        SelectionListIntent.putExtra("toolbar","Voice Over");
                        startActivity(SelectionListIntent);
                    }
                    else if(check.equals("anyone"))
                    {
                        Toast.makeText(VoiceOver.this,"Anyone Can Attend "+title+": )",Toast.LENGTH_LONG).show();
                    }
                    else
                    {
                        if(title==null)
                        {
                            Toast.makeText(VoiceOver.this, "Please wait while fetching the data", Toast.LENGTH_LONG).show();
                        }
                        else
                        {
                            Toast.makeText(VoiceOver.this, "Selection List For " + title + " is not yet updated", Toast.LENGTH_LONG).show();
                        }
                    }
                }
                else
                {
                    Toast.makeText(VoiceOver.this,"Please Check Your Internet Connection",Toast.LENGTH_LONG).show();
                }
            }
        });
    }
